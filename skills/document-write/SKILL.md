---
name: document-write
description: 技術・運用文書を、実装と既存文書に整合させて作成・更新する。問題点の指摘のみの場合は document-review を使用する。
---
...
文書の対象読者と用途を決めた後、執筆前に[文章作成ガイド](references/writing-guide.md)を読む。README、セットアップ手順、API文書、設計文書、運用手順書のいずれかを扱う場合は、該当する作業を始める前に[文書種別ガイド](references/document-types.md)も読む。出力前に、選択した文書種別の完了条件を再確認する。
...
6. 実装や運用手順との整合性に加え、用語の揺れ、直訳調、不必要な英語表現、リンク、見出し、Markdownの崩れ、コード例やコマンド例の再現性を確認する
...
- 修正は正規の情報源となる生成元で行い、生成物への直接修正は再生成で失われる前提で扱う
- 自動生成で反映されない補足説明がある場合は、手書き部分との責務を分ける
- 再生成手順や更新契機が必要な場合は文書内または周辺文書に残す
...
- ローカル開発、CI、本番運用など、読者ごとの差分がある場合は、誤読を避けるために分ける
...
- 実装から直接読み取れない推測は書かない
- 曖昧な表現を避ける
- 作業の記録や今後の方針を書く場合は、作業の方向性の提示にとどめない
...
- 分類名の下に説明が続く場合は、太字ラベル付きの箇条書きではなく、文書階層に合う見出しを使う
- 箇条書きは、同列の項目を複数並べる場合、手順を示す場合、確認事項や規約を列挙する場合に使う
- 単一の要点を述べる場合は段落で記載し、1 項目のみの箇条書きは避ける
